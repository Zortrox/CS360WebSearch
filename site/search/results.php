...
if ($querySplit[0] == "" && count($stringSearch[1]) == 0) { //if no query tell user
	...
	if ($sitesFound > 0) {
		...
		for ($i=$offset; $i<$sitesFound; $i++) {
			...
			$debugWeight = "";
			if ($doDebug) $debugWeight = " - " . $linkWeight;
			...
			echo "<p><a href='$url' class='title' style='font-family:ariel,sans-serif;'> <b> $title$debugWeight </b> </a> <br> $desc <br> <a href='$url' style='text-decoration:none;'> $url </a></p>";
		...
		for ($i=0; $i<$displayOffset; $i++) {
			...
			if ($writingPage > 0 && $writingPage <= $pages) {
				...
				else echo "<a href='?q=$query&p=$writingPage'>$writingPage</a> ";
			}
		}
		if ($page < $pages)
		{
